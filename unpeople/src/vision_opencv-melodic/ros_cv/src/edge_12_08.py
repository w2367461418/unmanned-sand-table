...
#将（170-720）行的点进行保存 
    ...
    for i in range(200):
        index = 359 - i
        if ll_[index] != 0 and ll_[index-5] != 0:   
            start_l = index
            break
    for i in range(200):
        index = 359 - i
        if rr_[index] != 639 and rr_[index-5] != 639:   
            start_r = index
            break
    for i in range(200):
        index = 159 + i
    ...
    for i in range(200):
        ...
        if rr_[index] != 639 and rr_[index+5] != 639:   
            end_r = index
            break
    yl1 = (int)(start_l - (start_l - end_l) * 0.2)
    xl1 = ll_[yl1] 
    yl2 = (int)(start_l - (start_l - end_l) * 0.6)
    xl2 = ll_[yl2] 

    yr1 = (int)(start_r - (start_r - end_r) * 0.2)
    xr1 = rr_[yr1] 
    yr2 = (int)(start_r - (start_r - end_r) * 0.6)
    xr2 = rr_[yr2] 
    ...
    count_l = start_l - end_l
    count_r = start_r - end_r


    print ('l:',xl1,yl1,xl2,yl2,kl,bl)
    print ('r:',xr1,yr1,xr2,yr2,kr,br)
         ...
         #       i[0] = (int)(-(index - 360) / 5.0)
            if (count_l >= count_r ):
                if (count_l <= 30):
                    ...
                    continue
                if(kl > -5 and kl < 0 and count_l > 30):
                  #  pass
                     i[0] = (int)((index - bl) / kl)
                else:
                    i[0] = (int)(-(index - 360) / 5.0)
       ...
       #         i[0] = (int)((index + 2768) / 5.0)
            if (count_r >= count_l ):
                if (count_r <= 30):
                    ...
                    continue
                if(kr < 5 and kr > 0 and count_r > 30):
                   # pass
                    i[0] = (int)((index - br) / kr)
                else:
                    i[0] = (int)((index + 2768) / 5.0)
