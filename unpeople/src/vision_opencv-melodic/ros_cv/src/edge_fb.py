...
#把轮廓上杂乱的点 提取成左右线（一行只有一个点）
    ...
    for index in range(500):
        index_now = 719 - index
        if count_r >= 50 and count_r - count_l > 10:
            if rr_[index_now] != 1279 and ll_[index] == 0:
                ll_[index_now] = rr_[index_now]
                rr_[index_now] = 1279
                continue 
        if count_l >= 50 and count_l - count_r > 10:
            if ll_[index_now] != 0 and rr_[index] == 1279:
                rr_[index_now] = ll_[index_now]
                ll_[index_now] = 0
                continue 
        if ll_[index_now] == 0:
            count_l = count_l + 1
        if rr_[index_now] == 1279:
            count_r = count_r + 1
    count_l = str(count_l)
    count_r = str(count_r)
...
#画出基准线
    ...
    actual_line = []
    mid_value = 0
    for i  in range(60):  #计算60行的中值 求平均
        mid_value = mid_value + mid[300+i][0]
        pose = (640,300+i)
        reference_line.append(pose)
    mid_value = int(mid_value / 60) #当前小车的中直
